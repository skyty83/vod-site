...
function SearchContent() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    searchVideos(query, page).then(result => {
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 flex-1 w-full">
      ...
      <Link
        href="/"
        className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors text-sm font-bold mb-6 group bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/50 dark:hover:bg-slate-800 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-800 shadow-sm w-fit"
      >
      ...
      <div className="relative mb-10 overflow-hidden rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl p-6 sm:p-10 lg:p-14 text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto flex flex-col items-center">
          <div className="inline-flex items-center justify-center p-3 sm:p-4 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 shadow-xl mb-6 text-white self-center mx-auto">
          ...
          <form onSubmit={handleSearchSubmit} className="w-full relative group shadow-2xl">
            ...
            <div className="relative flex items-center bg-white/10 dark:bg-black/40 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-2xl overflow-hidden transition-all duration-300 group-focus-within:border-white/40">
              <Search
                size={24}
      ...
      {/* Results Section */}
      {!query ? (
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center bg-white/50 dark:bg-slate-900/40 rounded-3xl border border-slate-200 dark:border-slate-800">
          <div className="w-20 h-20 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-6 shadow-inner text-slate-400">
            <Search size={40} />
          </div>
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-2">输入片名开始搜索</h3>
          <p className="text-slate-500 dark:text-slate-400 text-sm max-w-xs mx-auto">千万影视资源任你搜索，即搜即看，极速秒播</p>
        </div>
      ) : (
        <div className="flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
            ...
            {!loading && total > 0 && (
              <span className="text-slate-600 dark:text-slate-300 text-sm font-medium bg-white dark:bg-slate-800/80 px-4 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-2">
                <Flame size={16} className="text-rose-500" />
                共找到 <strong className="text-slate-900 dark:text-white text-base">{total.toLocaleString()}</strong> 部
              </span>
            )}
          </div>
